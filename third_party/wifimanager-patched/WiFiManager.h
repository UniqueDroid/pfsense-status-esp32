...
class WiFiManager
{
  public:
    ...
    // custom status html shown below wifi connection status on pages that render status
    void          setCustomStatusHTML(const char* html);
    ...
    // show back button on all sub-pages
    void          setShowBack(boolean enabled);
    ...
    // optional password gate for web portal (empty pass disables)
    void          setHttpAuth(const char *user, const char *pass);
  ...
  protected:
    ...
    const char*   _customStatusHTML       = ""; // custom html rendered below wifi status
    String        _bodyClass              = ""; // class to add to body
    String        _title                  = FPSTR(S_brand); // app title -  default WiFiManager
    String        _httpAuthPass           = "";
    bool          _httpAuthEnabled        = false;
    IPAddress     _httpAuthClientIp;
    unsigned long _httpAuthUntil          = 0;
